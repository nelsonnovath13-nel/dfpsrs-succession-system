...
function NewPropertyForm() {
  ...
  const [phase, setPhase] = useState<"count" | "form">("count");
  ...
  function startForm(e: React.FormEvent) {
    e.preventDefault();
    const n = Math.max(1, Math.min(20, Number(targetCount) || 1));
    setTotal(n);
    setSavedCount(0);
    setPhase("form");
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const justSaved = savedCount + 1;
      if (justSaved < total) {
        // More properties left in this batch -- reset the wizard for the next one.
        setSavedCount(justSaved);
        setForm(EMPTY_FORM);
      ...
      // Navigate immediately once the whole batch is saved -- no extra query gates this,
  ...
  return (
    <DashboardShell role="owner">
      ...
      {phase === "count" ? (
        <form onSubmit={startForm} className="card max-w-md space-y-4">
          <div>
            <label className="label">{sw ? "Ni mali ngapi unataka kusajili sasa?" : "How many properties do you want to register now?"}</label>
            <input
              type="number"
              required
              min={1}
          ...
          <button type="submit" className="btn-primary">{sw ? "Endelea" : "Continue"}</button>
        </form>
      ) : (
        <div className="card max-w-xl">
          {total > 1 && (
            <p className="text-sm font-medium text-primary mb-3">
              {sw ? `Mali ya ${savedCount + 1} kati ya ${total}` : `Property ${savedCount + 1} of ${total}`}
            </p>
          )}
            ...
            {step < STEP_LABELS.length - 1 ? (
              ...
              <button type="button" onClick={handleSubmit} disabled={loading} className="btn-primary">
                {loading
                  ...
                  : (sw ? "Hifadhi na Sajili" : "Save Property")}
              </button>
            )}
          </div>
        </div>
