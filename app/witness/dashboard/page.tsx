...
export default function WitnessDashboardPage() {
  ...
  return (
    <DashboardShell role="witness">
      ...
      <h2 className="text-sm font-semibold text-neutralDark uppercase mb-3">Awaiting Your Review</h2>
      {loading ? (
        <p className="text-sm text-neutralDark">Loading…</p>
      ) : pending.length === 0 ? (
        <div className="card text-center py-8 text-neutralDark mb-8">No pending requests.</div>
      ) : (
