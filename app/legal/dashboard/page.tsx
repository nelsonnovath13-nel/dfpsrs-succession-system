...
export default function LegalDashboardPage() {
  ...
  return (
    <DashboardShell role="legal">
      <h1 className="text-xl font-semibold text-primary mb-6">{sw ? "Kituo cha Uhakiki wa Kisheria" : "Legal Review Center"}</h1>

      <h2 className="text-sm font-semibold text-neutralDark uppercase mb-3">{sw ? "Inasubiri Uhakiki wa Kisheria" : "Awaiting Legal Review"}</h2>
      {loading ? (
        <p className="text-sm text-neutralDark">{sw ? "Inapakia…" : "Loading…"}</p>
      ) : pending.length === 0 ? (
        <div className="card text-center py-8 text-neutralDark mb-8">{sw ? "Hakuna kumbukumbu zinazosubiri uhakiki wa kisheria." : "No records pending legal review."}</div>
      ) : (
